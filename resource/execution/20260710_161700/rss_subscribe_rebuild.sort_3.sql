CREATE TABLE IF NOT EXISTS rss_subscribe_new (
    ...
    `name_vector` vector(512) DEFAULT NULL,
    CONSTRAINT `idx_name` UNIQUE (`name`),
    CONSTRAINT `idx_season_cover` UNIQUE (`season`, `cover`)
);

INSERT INTO rss_subscribe_new (
    id, name, name_jp, url, regex, season, start_time, cover,
    fin, is_short, anime_type, goon, staff, `cast`, origin_type, type_tag, broadcast
)
SELECT
    id, name, name_jp, url, regex, season, start_time, cover,
    fin, is_short, anime_type, goon, staff, `cast`, origin_type, type_tag, broadcast
...
-- CREATE INDEX idx_rss_subscribe_name_vector ON rss_subscribe USING libsql_vector_idx(name_vector);