...
export default {
    selectBySubsId: (subsId) => {
        const sql = 'SELECT id, rss_subs_id, rss_result_id, status FROM rss_torrent_task ' +
            'WHERE rss_subs_id = ?'
        return sqliteDB.selectAll(sql, [subsId], null, dbName)
    },
    ...
    selectExistsByResultId: (rssResultId) => {
        ...
        return sqliteDB.selectOne(sql, [rssResultId], null, dbName).then(data => data.count > 0)
    },
    selectByUUID: (uuid) => {
        const sql = 'SELECT id, rss_subs_id as rssSubsId, rss_result_id as rssResultId, torrent_uuid as uuid, torrent_hash as hash, status FROM rss_torrent_task ' +
            'WHERE torrent_uuid = ?'
        return sqliteDB.selectOne(sql, [uuid], null, dbName)
    },
    insertOne: (rssTask, transactionDB) => {
        ...
        return (transactionDB || sqliteDB).insert(sql, params, null, dbName)
    },
    updateStatusByUUID: (uuid, status) => {
        const sql = 'UPDATE rss_torrent_task SET status = ? WHERE torrent_uuid = ?'
        return sqliteDB.update(sql, [status, uuid], null, dbName)
    },
    ...
    updateTaskHashById: (id, hash) => {
        ...
        return sqliteDB.update(sql, [hash, id], null, dbName)
    },
    selectByIds: async (ids) => {
        ...
        if (excludes.length > 0) {
            ...
            const data = await sqliteDB.selectAll(sql, excludes, null, dbName).then(({ data }) => data)
            if (data.length > 0) {
                saveCache(data)
                results.push(...data)
            }
