...
export default {
    ...
    insertOne: (rssLink) => {
        ...
        return __sqliteDB.insert(sql, [rssLink.pid, rssLink.title, rssLink.href], enablePrint, dbName);
    },
    insertMany: (rssLinkArr, transactionDB) => {
        if (__isEmptyArray(rssLinkArr)) return Promise.resolve();
        const params = [];
    ...
    updateOne: (rssLink) => {
        ...
        return __sqliteDB.update(sql, [rssLink.title, rssLink.href, rssLink.id], enablePrint, dbName);
    },
    deleteOne: (id) => {
        const sql = "DELETE FROM rss_link WHERE id=?";
        return __sqliteDB.delete(sql, [id]);
    },
