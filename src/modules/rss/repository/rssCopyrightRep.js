...
export default {
    selectByPid: (pid) => {
        ...
        return __sqliteDB.selectAll(sql, [pid], enablePrint, dbName);
    },
    selectByPidV2: (pid) => {
    ...
    selectPidById: (id) => {
        ...
        return __sqliteDB.selectOne(sql, [id], enablePrint, dbName).then(res => res.pid);
    },
    insertOne: (rssCopyright) => {
        const sql = "INSERT INTO rss_copyright (pid, area, href, image) VALUES (?, ?, ?, ?)";
        return __sqliteDB.insert(sql, [rssCopyright.pid, rssCopyright.area, rssCopyright.href, rssCopyright.image], enablePrint, dbName);
    },
    insertMany: (rssCopyrightArr, transactionDB) => {
        if (__isEmptyArray(rssCopyrightArr)) return Promise.resolve();
        const params = [];
    ...
    updateOne: (rssCopyright) => {
        ...
        return __sqliteDB.update(sql, [rssCopyright.area, rssCopyright.href, rssCopyright.image, rssCopyright.id], enablePrint, dbName);
    },
    deleteOne: (id) => {
    ...
    deleteManyByPid: (pid, transactionDB) => {
        ...
        return (transactionDB || __sqliteDB).delete(sql, [pid], enablePrint, dbName);
    },
    deleteManyByPids: (pids, transactionDB) => {
        ...
        return (transactionDB || __sqliteDB).delete(sql, pids, enablePrint, dbName);
    },
    selectAllImage: () => {
        const sql = "SELECT image FROM rss_copyright GROUP BY image";
        return __sqliteDB.selectAll(sql, [], enablePrint, dbName).then(res => res.data.map(obj => obj.image));
    }
}