...
import { updateLatestConfig } from './clashBackupService.js';
...
/**
 * 聚合订阅源与本地模版规则，生成最新的 Clash 配置文件
 * 1. 读取基础模版 basic-conf.yaml
 * 2. 注入各订阅源代理节点、执行关键词过滤与策略组动态扩展
 * 3. 混入本地策略组 mixin-conf.yaml 与前置规则 prefix-conf.yaml
 * 4. 展开外部 rule-providers 规则集
 * 5. 执行规则去重并持久化输出
 */
export function concatClashYaml() {
    const concatPath = __join(__env.get('clash.path.concat', '@/'));
    if (!fs.existsSync(concatPath)) {
        __throwMessage('Clash concatPath not exists.');
    }
    const sources = getSubscriptionSourcesObj();
    if (sources.length === 0) {
        __throwMessage('Clash sources is empty.');
    }
...
/**
 * 将订阅源节点、过滤规则、策略组映射及前置规则混入到基础配置中
 * @param {ClashConfig} content - 基础配置
 * @param {string} concatPath - 模版目录路径
 * @param {ClashSubscriptionSourceObj[]} [sources=[]] - 订阅源对象列表
 * @returns {ClashConfig} 合并处理后的配置对象
 */
function mixinModule(content, concatPath, sources = []) {
    ...
    content['proxy-groups'].forEach(g => {
        ...
        let lastIndex = 0;
        const result = Array.from(proxiesConcat.keys()).reduce((arr, index) => {
...
/**
 * 混入 mixin-conf.yaml 中的策略组与规则定义
 * @param {ClashConfig} content
...
function handleMixinYaml(content, concatPath) {
    ...
    if (!fs.existsSync(CONFIG_MIXIN)) return;
...
/**
 * 读取并解析所有 rule-providers 引用的本地 yaml 规则集 payload
 * @param {ClashConfig} content
 * @param {string} concatPath
