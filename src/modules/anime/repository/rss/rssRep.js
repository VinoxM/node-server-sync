import { rssSubscribe, rssResult } from '../../entity/rssResultMap.js';
import { isCurSeason } from '#utils/dateUtil.js';
...
export default {
    selectRssSubscribeBySeasonAndSearch: (season, search) => {
        ...
        return __sqliteDB.selectAll(sql, params, { ...enablePrint, resultMap: rssSubscribe }, dbName);
    },
    ...
    selectRssSubscribeSeasons: () => {
        ...
        return __sqliteDB.selectAll(sql, [], enablePrint, dbName);
    },
    ...
    selectRssResultsByPid: (pid, withOutHide) => {
        const sql = "SELECT id,pid,title,torrent,pub_date,hide,tracker,episode,up_date,sort FROM rss_result WHERE pid=? " +
            ...
            "ORDER BY sort DESC";
        return __sqliteDB.selectAll(sql, [pid], { ...enablePrint, resultMap: rssResult }, dbName);
    },
    selectRssResultsByPidV2: (pid) => {
        const sql = "SELECT id, title,torrent,pub_date,tracker,episode FROM rss_result WHERE pid=? AND hide=0 ORDER BY sort DESC";
        return __sqliteDB.selectAll(sql, [pid], { resultMap: rssResult }, dbName);
    },
    ...
    selectRssSubscribeCauseGoon: (season) => {
        const sql = "SELECT rs.id,rs.name,rs.name_jp nameJP,rs.regex,rs.url,rs.season,rs.start_time " +
            ...
            "GROUP BY rs.id";
        return __sqliteDB.selectAll(sql, [season], { ...enablePrint, resultMap: rssSubscribe }, dbName);
    },
    ...
    selectOneById: (id) => {
        const sql = "SELECT name, name_jp, cover,fin,is_short,staff,[cast],origin_type originType,type_tag typeTag,broadcast,season FROM rss_subscribe WHERE id = ?";
        return __sqliteDB.selectOne(sql, [id], { resultMap: rssSubscribe }, dbName);
    },
    selectOneForEdit: (id) => {
