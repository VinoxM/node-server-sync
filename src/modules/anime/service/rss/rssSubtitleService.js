...
import { copyRemoteFileToMinio, removeRemoteFiles } from '#modules/ssh/sshExecutorService.js';
...
/**
 * 解析并处理单条字幕文件（入库、转存 MinIO、转换 SRT 为 VTT、扫描 ASS 字体）
 * @param {number} taskId - 关联任务 ID
 * @param {number} subsId - 关联订阅 ID
 * @param {string} fileName - 字幕相对文件名
 * @param {string} rootPath - 本地根目录
 * @param {string} season - 所属季度
 * @param {string} animeName - 动画名称
 * @param {string} simpleFileName - 文件基名
 * @param {string|number|null} [useEpisode=null] - 显式指定的集数
 * @returns {Promise<{ complete: boolean, fileRemoved: boolean, missingFonts: string[], subtitleId: number|null }>}
 */
export async function resolveEpisodeSubtitle(taskId, subsId, fileName, rootPath, season, animeName, simpleFileName, useEpisode = null) {
    ...
    const episode = useEpisode ?? getAnimeEpisode(simpleFileName);
    if (episode !== null) {
 ...
 * 回填替换字幕中的字体名称映射
 * @param {number} id - 字幕 ID
 * @param {Record<string, string>} backfill - 字体替换字典
 * @returns {Promise<void>}
 */
